...
const databasePath = "/tmp/tempo-playwright.db";
...
export default defineConfig({
  ...
  webServer: {
    command: `rm -f ${databasePath} && npm run build && ../.venv/bin/alembic -c ../alembic.ini upgrade head && ../.venv/bin/uvicorn tempo.main:app --host 127.0.0.1 --port 8001`,
